...
class LSM6DSOX:
    ...
    def __init__(
        ...
        cs_pin=None,
        address=0x6b,
        gyro_odr=104, #자이로 샘플링 속도
        accel_odr=104, #가속도 샘플링 속도
        gyro_scale=2000, 
        accel_scale=4,
    ):
        ...
        """
        self.bus = bus #자동으로 세팅?
        self.cs_pin = cs_pin
        self.address = address
        self._use_i2c = hasattr(self.bus, "readfrom_mem") #self.bus에 ""존재하는지 확인 

        if not self._use_i2c and cs_pin is None:
            raise ValueError("A CS pin must be provided in SPI mode")

        #Accelerometer/Gyro ID체크 
        if self.__read_reg(_WHO_AM_I_REG) != 106:
            print(self.__read_reg(_WHO_AM_I_REG))
